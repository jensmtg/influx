...
- [x] Add lifecycle/integration tests for `src/features/editor/codemirror/async-view-plugin.ts`. We covered `stateful-decoration-set` and widget cleanup, but the wrapper that handles file switches, debounce, hide/show, and destroy cleanup is still basically unguarded.
